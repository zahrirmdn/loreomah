...
const EditSlider = () => {
  ...
  const [slider, setSlider] = useState({ title: "", description: "", image: "" });
  ...
  useEffect(() => {
    const fetchSlider = async () => {
      ...
      try {
        const res = await API.get(`/api/sliders/${id}`);
        const data = res.data;
        setSlider({
          title: data.title || "",
        ...
        const errorMsg = error.response?.data?.detail || error.message || "Gagal memuat data slider";
  ...
  const handleSave = async () => {
    if (!slider.title || !slider.description) {
      toast({ title: "Error", description: "Judul dan deskripsi wajib diisi.", variant: "destructive" });
      return;
    }
    ...
    try {
      await API.put(`/api/sliders/${id}`, formData, { headers: { "Content-Type": "multipart/form-data" } });
      toast({ title: "Berhasil!", description: "Slider berhasil diperbarui." });
        ...
        {fetchLoading ? (
          <Card>
            <CardContent className="py-8 sm:py-12">
              <p className="text-center text-gray-500 text-sm">Memuat data slider...</p>
            </CardContent>
          </Card>
        ) : (
          <Card>
            <CardHeader>
              <CardTitle className="text-base sm:text-lg">Edit Slider</CardTitle>
              <CardDescription className="text-xs sm:text-sm">Perbarui informasi slider hero</CardDescription>
            </CardHeader>
          <CardContent className="p-3 sm:p-6">
            <div className="space-y-4 sm:space-y-6">
              {/* Title */}
              <div>
                <label className="block text-xs sm:text-sm font-medium text-gray-700 mb-1.5 sm:mb-2">Judul Slider</label>
                <Input
                  placeholder="Contoh: Selamat Datang di Cafe Loreomah"
                  value={slider.title}
              ...
              <div>
                ...
                <Textarea
                  placeholder="Nikmati kopi terbaik dengan suasana nyaman"
